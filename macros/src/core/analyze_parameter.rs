...
pub fn analyze_parameter(pat_type: &PatType) -> ParamInfo {
	...
	let kind =
		if ty_str.contains("MurRequestContext") || ty_str.contains("MurReq") {
			ParamKind::Context
		} else if ty_str.starts_with("MurJson<")
			|| ty_str.starts_with("murgamu::MurJson<")
		{
			let inner = extract_generic_type(&ty_str, "MurJson");
			ParamKind::Json(inner.parse().unwrap_or(quote!(serde_json::Value)))
		} else if ty_str.starts_with("MurQuery<")
			|| ty_str.starts_with("murgamu::MurQuery<")
		{
			let inner = extract_generic_type(&ty_str, "MurQuery");
			ParamKind::Query(
				inner.parse().unwrap_or(
					quote!(std::collections::HashMap<String, String>),
				),
			)
		} else if ty_str.starts_with("MurPath<")
			|| ty_str.starts_with("murgamu::MurPath<")
		{
			let inner = extract_generic_type(&ty_str, "MurPath");
			ParamKind::Path(inner.parse().unwrap_or(quote!(String)))
		} else if ty_str.starts_with("Param<")
			|| ty_str.starts_with("murgamu::Param<")
		{
		...
		// }
			...
			ParamKind::Parts
		} else if ty_str.contains("Option<Bytes>")
			|| ty_str.contains("Option<hyper::body::Bytes>")
		{
			ParamKind::RawBody
		} else if ty_str.contains("Arc<MurServiceContainer>")
			|| ty_str.contains("MurServiceContainer")
		{
			ParamKind::Container
